...
const Following = ({ navigation }) => {
    ...
    useEffect(() => {
        dispatch(UserActions.getFollowingRequest(userData?.userData?.login))
    }, [userData?.userData])

    return (
        <View style={{ width: '100%', height: '100%', backgroundColor: Colors.background }}>
            {
                userData.followingLoading ? <ActivityIndicator color={'white'} size={'large'} /> : (
                    <>
                        <View style={styles.header}>
                            ...
                            <View style={{ width: '100%', alignItems: 'center' }}>
                                <Text style={styles.hashtag}>
                                    {userData?.userData?.following + ' seguindo'}
                                </Text>
                            </View>
                        </View>

                        <FlatList
                            data={userData.followingData}
